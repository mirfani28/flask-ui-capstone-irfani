...
playstore['Installs'] = pd.to_numeric(playstore['Installs'], downcast="integer")
playstore['Price'] = pd.to_numeric(playstore['Price'], downcast="float")

playstore['Reviews'] = pd.to_numeric(playstore['Reviews'], downcast="integer")
playstore['Size'] = pd.to_numeric(playstore['Size'], downcast="integer")
playstore['Installs'] = pd.to_numeric(playstore['Installs'], downcast="integer")
...
def index():
    ...
    ## Plot untuk menunjukan Kategori Aplikasi yang paling menguntungkan
    df2['Profit'] = df2['Installs'] * df2['Price']
    cat_order_irfani = df2.groupby(by='Category').agg({
    'Profit' : 'sum' }).sort_values(by='Profit',ascending=False).head(8).reset_index()
    X = cat_order_irfani['Category']
    Y = cat_order_irfani['Profit']
    my_colors = 'rgbkymc'
    # bagian ini digunakan untuk membuat kanvas/figure
    ...
    plt.barh(X ,Y , color=my_colors)
    # bagian ini digunakan untuk menyimpan plot dalam format image.png
    plt.savefig('cat_order_irfani.png',bbox_inches="tight") 
